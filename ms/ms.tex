...
\keywords{growth rate $|$ traits $|$ trees $|$ wood density}
...
\textbf{Change in growth rate with increasing size (Fig. \ref{fig:conceptual})}\\
 ...
 \textsc{rgr} (all variables): decreasing \citep{Rees-2010, Iida-2014}\\
\textbf{Effect of traits on growth rate (Fig. \ref{fig:growth_light_height})}\\
 $\omega$: low values $\rightarrow$ smaller seedlings, lower absolute, higher relative \citep{Gibert-2016} \\
...
\textbf{Shade tolerance, \textsc{wplcp} (Fig. \ref{fig:wplcp})}\\
 ...
 $\rho$: low values less shade tolerant \citep{Osunkoya-1996}\\
  \bottomrule
  \end{tabular}
\addtabletext{The traits are: seed mass ($\omega$), height at maturation ($H_{{\rm mat}}$), leaf nitrogen content per unit leaf area ($\nu$)$^1$, leaf mass per area ($\phi$)$^2$, and wood density ($\rho$). See Tables \ref{tab:definitions} and \ref{tab:traits} for further details. textsc{lai} = leaf area index. Footnotes: $^1$ Similar responses are predicted for maximum photosynthetic rate per leaf area \& dark respiration rate per leaf area. Here these terms are directly related to $\nu$. $^2$ Similar responses are predicted for leaf lifespan. Here this terms is directly related to $\phi$. $^3$ \citep{Baltzer-2007} reports a strongs relationship between \textsc{wplcp} and respiration rate measured per unit leaf area and mass.}
\label{tab:phenomena}
\end{table}
...
adapted from \citep{Givnish-1988}.
...
\begin{table}[!hb]
 \caption{Variables of the \texttt{FF16} physiological model from {\plant} used in this analysis.}
\centering
...
\bottomrule
\end{tabular}
 \addtabletext{For mass ($M$), respiration ($r$), and turnover ($k$)
...
\centering
  \begin{tabular}{p{0.5cm}p{3.0cm}p{2.5cm}p{1cm}}
  \toprule
...
\centering
  ...
  \bottomrule
\end{tabular}
\addtabletext{\\The first column of part \textbf{a} provides cores assumptions between various size metrics and leaf area. Eqns in the middle and right columns of \textbf{b}, and in \textbf{b} can then be derived from the assumptions in the left column of \textbf{a}.
...
The {\plant} package adopts a model of plant construction and function that can be considered a first-order functional-balance or function-equilibrium model, similar to those implemented by \citep{Makela-1997} and \citep{Moorcroft-2001}. We could also call it ``isometric'', because the assumptions see area-based metrics scaling to the first-power of other area-based metrics, and to the square-power of length-based metrics, such as height \citep{Huxley-1932}. Table \ref{tab:allometry} provides key equations of the model (see \citealt{Falster-2016} for full derivation). In particular, we assume that as a plant grows:
\textbf{1)} Its height scales to the 0.5 power of its leaf area (eqn \ref{eq:ha});
\textbf{2)} The cross-sectional area of sapwood in the stem is proportional to its leaf area (eqn \ref{eq:as});
\textbf{3)} The cross-sectional area of bark and phloem in the stem  is proportional to its leaf area (eqn \ref{eq:ab});
\textbf{4)} The cross-sectional area of root surface area and therefore mass is proportional to its leaf area (eqn \ref{eq:mr}); and \textbf{5)} The vertical distribution of leaf within the plant's canopy, relative to the plant's height, remains constant.
...
\begin{figure}[!hb]
\centering
\includegraphics[width=.95\linewidth]{main/assumptions.pdf}
...
\begin{table}[t!]
\centering
\caption{Predicted effects of traits on key elements of plant function determining growth rate.}
...
\midrule
 \multicolumn{8}{l} {\textbf{b) Predicted effect of trait on growths rate for a small and large plant}} \\
  ...
  \bottomrule
  \end{tabular}
\addtabletext{Arrows indicate the effect an increase in trait value would have on each element of the equations, with dashes indicating no effect. See Table \ref{tab:traits} for trait definitions. Adapted and expanded from \citep{Gibert-2016}.}
\label{tab:responses}
\end{table}
