...
% XX - The main comment I have is whether the strength of the 'correlation' between traits and growth rates can be summarized in some metric, so that it can be directly shown. To me Fig 4 is not very clear on showing how size affects the trait-growth correlations. For example, for wood density, I see a relation for all size classes. For the largest trees, it may look like a weaker correlation but isn't that because growth rates are lower? The same with Narea, to me they all look the same (except again, larger trees grow slower but it’s hard to tell there for me whether the trait-growth correlation is any different)
% XX -  The shade tolerance (WPLCP) analysis seems off-topic (it also not mentioned in the abstract). Can this be integrated more closely with the main objective, i.e. correlation between traits and growth rate and its dependence on size?
...
% Could cite Blackman 1919 "The Compound Interest Law and Plant Growth" AnnBot - this approach is very old.
...
% Is this a direct biophysical consequence of varying this trait, as outlined as a necessary condition in the above?
...
% Importantly, we can remove these size dependent effects by altering ??? of the assumptions in Table \ref{tab:allometry} (??).
...
% Remko thinks the following sentence is over the top but Martin de K thought it was fine.
% :) 
...
% yes, I like this shortened version! 
