...
Plant growth is a fundamental process driving the terrestrial biosphere and much of human activity. While most higher plants have the same basic resource requirements (light water, nutrients) and physiological bauplan [@niklas_evolution_2000], key quantitative differences exist in the quality of construction and allocation to key plant components (leaves, stems, roots) [@westoby_plant_2002]. Leaf construction cost ($\phi$) - the amount of mass used to construct a unit of leaf area - is one such axis of variation, varying approx 40 fold among species  [@wright_world-wide_2004]. Low $\phi$ can, at times, accelerate growth, as leaves which are cheap to build enable faster canopy expansion [@westoby_time_2000]. Indeed, growth rate in seedlings is strongly and negatively correlated with $\phi$ (Fig \ref{}a)[@wright_cross-species_2000]. It has been proposed that low $\phi$ would also give a growth advantage in large plants [@enquist_general_2007], and much of research about plant strategy theory assumes this to be true (examples??).  However, a growing number of empirical studies have failed to detect any significant relationship between growth and $\phi$ in large trees (Fig \ref{}b)[@wright_functional_2010, @herault_functional_2011, @poorter_are_2008]. The growing discord between theoretical expectation and empirical result, indicates a strong need for a new theoretical framework through which the influence of $\phi$ on plant growth can be reassessed.

Here we extend the theoretical model used to link $\phi$ with growth rate in seedlings to explicitly include influences of size, light environment, and effects of leaf turnover, and thereby generate XX new predictions on the link between $\phi$ and plant demography (Table \ref{tab:predictions}). The new model captures the observed decrease in correlation between $\phi$ and growth rate with increasing size (Fig. \ref{f-RGR}), and also shows how $\phi$ determines shade tolerance (Fig. \ref{f-RGR}), and increases through ontogeny for all species (Fig. \ref{f-RGR}). Together, these findings provide...

We begin with the standard  model for net production in plants $dP \ \textrm{d}t$, given by the difference between total photosynthesis, respiration and turnover for the different plant components, accounting also for losses due to growth respiration:
...
\end{equation}
Here, $m,r$, and $k$ refer to the mass, respiration rate, and turnover rate of different tissues with subscripts $l$=leaves, $b$=bark, $s$=sapwood and $r$=roots, while $A$ is the assimilation rate per leaf area and $Y$ is yield (see Table ?? for units). Photosynthesis is proportional to leaf area, $a_\textrm{l} = m_\textrm{l}\phi^{-1}$, while total mass of living tissues $m_\textrm{t}=m_\textrm{l}+m_\textrm{b}+m_\textrm{s}+m_\textrm{r}.$

Current theory emphasises the relationship between $\phi$ and relative growth in mass for seedlings, $R_{m_\textrm{t}}= \textrm{d}P / \textrm{d}t \, / \, m_\textrm{t}$. For seedlings, which are young and mostly leaf, it is reasonable to ignore all turnover terms and respiration terms for non-leaf tissues. Net mass production then becomes a linear function of leaf area and net production per leaf area ($A_\textrm{net} = Y(A - r_\textrm{l})$) and $R_{m_\textrm{t}}$ a linear function of $\phi$:
\begin{equation}\label{eq:RGR}
R_{m_\textrm{t}} \approx A_\textrm{net} \times \phi^{-1} \times \frac{m_\textrm{l}}{m_\textrm{t}}. \end{equation}
Although eq. \ref{eq:RGR} fits data from seedlings
[@wright_cross-species_2000], where mass can be measured, it provides only an indirect link to the types of data routinely collected for large trees, namely basal area or height growth. More fundamentally, the predicted link between $\phi$ and growth is absent in larger plants (Fig xx).

To properly model height ($h$) or basal area ($a_\textrm{st}$) growth rate, it becomes necessary to also account for the costs of building new tissues, allocation to reproduction, and architectural layout. This is achieved mathematically by decomposing height growth rate into a product physiologically relevant terms:
...
The first term in eq \ref{eq:dhdt}, $\textrm{d}h / \textrm{d}a_\textrm{l}$, is the growth in height per unit growth in leaf area; accounting for the architectural strategy of the plant. (Some species tend to leaf out more than grow tall, while other species emphasise vertical extension.)

The second term $\textrm{d}a_\textrm{l} / \textrm{d}m_\textrm{t}$ accounts for the cost of deploying a unit of leaf area, including construction of the leaf itself and various support structures required, and as such can be further decomposed as the inverse of a sum of construction costs per unit leaf area:
...
Eqs.  \ref{eq:dhdt} and \ref{eq:daldmt} are mathematically true and thus hold for any model of plant growth. To make explicit predictions about the influence of traits on growth, however, requires two additional features: 1. A specific model describing how the various mass and area terms ($m_\textrm{l}, m_\textrm{s}, m_\textrm{b}, m\textrm{r}, a_\textrm{l}, a_\textrm{s}, a_\textrm{b}$) vary relative to one another; and 2. for the effects of varying $\phi$ on plant function to be specified. To address the first of these, we use a simple allometric model [@yokozawa_foliage_1995; @falster_influence_2011] assuming:

1. An allometric relation between total leaf area and plant height,
2. The pipe model, relating cross-sectional area of sapwood and bark to total leaf area, and
3. A fixed ratio of root mass per leaf area.

These assumptions, which are well supported by data (Fig. XX), lead immediately to the set of equations for plant size described in table Table \ref{tab:allometry} (see Supplementary material for derivations).

In a similar way, basal area increment can be expressed as the sum of increments in sapwood, bark \& heartwood area, $\textrm{d}a_\textrm{st} / \textrm{d}t= \textrm{d}a_\textrm{ss} / \textrm{d}t + \textrm{d}a_\textrm{b} / \textrm{d}t + \textrm{d}a_\textrm{sh} / \textrm{d}t$, which by the assumptions above also become functions of leaf area (Table ?). Thus:
...
The direct effect of increasing $\phi$ on growth is to increase the marginal cost of building leaf area  (eq. \ref{eq:daldmt}) and thus reduce growth rate. However, to fully account for the influence of $\phi$ on growth we must also account for benefits of superior leaf construction: increased leaf longevity [@wright_world-wide_2004]. This is achieved by linking the turnover rate of leaf in eq. \ref{eq:dPdt} to $\phi$. We use an allometric scaling relationship of the form $k_\textrm{l}=\alpha_4 \, \phi^{\beta_4}$ as this gives a good fit to the data  (Fig. \ref{fS-leaf}).

Unlike eq. \ref{eq:RGR} which includes a fixed linkage between $\phi$ and growth, the model shows how this relationships is moderated by plant size. Decreasing $\phi$ has two impacts on either height or basal area growth: increasing the leaf deployment rate ($\textrm{d}a_\textrm{l} / \textrm{d}m_\textrm{t}$, by economising on construction costs) and decreasing net production ($\textrm{d}P / \textrm{d}t$, due to increased leaf turnover). Whether lower $\phi$ increases growth thus depends on the relative size effects  of these two effects. When plants are small the effect on leaf deployment rate is bigger and so decreasing $\phi$ increases growth rate. When plants are large, the influence of $\phi$ on leaf deployment rate is diminished, because the costs of building other supportive tissues (other terms in eq. \ref{eq:daldmt}) are larger. The net result is that at larger sizes, low $\phi$ is no longer advantageous (for growth).
