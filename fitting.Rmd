...
run_tree <- function(x, E=1, traits=list(), pars=list(), strategy = default_strategy()) {

  distance_from_target_fn <- function(plant) {
    plant$vars_size[["diameter"]] - x
  }

  strategy <- strategy$copy()
  strategy$set_parameters(traits)
  strategy$set_parameters(pars)
  plant1 <- new(Plant, strategy)
  plant1$height <- 0.1
  plant <- try(grow.plant.to.size(plant1, fixed.environment(E), distance.from.target = distance_from_target_fn), silent = FALSE)
  if (inherits(plant, "try-error"))
      return(0)
  run_plant(plant, E)$dbasal_diam_dt
}
```
...
getPred <- function(pars, xx) {
	...
	for(i in seq_len(npoints)) {
		zz[i] <- run_tree(xx$dbh[i], E=1, traits=list(lma=xx$lma[i], rho=xx$rho[i], hmat=100), pars=pars)
	}
	zz
}
```
...
The set parameters to fit an some lower bounds (optionl) for these:
...
getPred(par_init, x_example)
residFun(par_init, obs_example, x_example)
```
...
nls.out <- nls.lm(par=par_init, fn = residFun, observed = obs, xx = x,
		lower=lower,
		control = nls.lm.control(nprint=1, ftol=1e-4))

out.init <- getPred(par_init, x)
out.fit <- getPred(as.list(coef(nls.out)), x)
```
...
for(trait in c("lma", "rho")) {
	...
	out.init <- getPred(par_init, xs)
	out.fit <- getPred(as.list(coef(nls.out)), xs)
...
Using the nls is ok but not ideal because it only returns one plausible combination of parametrs. A better approach would be to fit using a
distribution-based approach. For this there are two challenges:
