...
run_tree <- function(x, E=1, traits=list(), pars=list(),
  ...
  pl <- run_plant_to_sizes(x, "diameter", strategy, env, time_max=3000)


  strategy=FF16_Strategy()
  env <- fixed_environment(1)  
  pl <- FF16_PlantPlus(strategy)
  res <- grow_plant_to_size(pl, x, "diameter", env, time_max=3000)


  x <- seq(FF16_Plant(strategy)$height, strategy$hmat, length.out=100)
...
run_plant_to_sizes
  ...
  pp <- grow_plant_to_size(pl, x, "diameter",
                           fixed_environment(E))
