...
#Look up family
lookup_family <- function(tag){
  nomen <- read.csv('data/Nomenclature_R_20101129_Rready.csv',
...
flag_bad_data <- function(dbh, dbh_increment, dbh_dt) {
  ...
  accept[!is.na(dbh_dt) & dbh_dt > max_growth] <- FALSE
...
BCI_calculate_individual_growth <- function(data_in) {
  ...
  data <- data_in %>% 
    ...
    mutate(spcode=lookup_spcode(Latin), # adds species code column
           family = lookup_family(spcode)) # adds family column
  ...
  individual_growth <- data %>%
    ...
    filter((hom == 1.3 | status=='dead') &
             !family %in% c('Arecaceae', 'Cyatheaceae', 'Dicksoniaceae', 'Metaxyaceae', 
                            ...
                            'Thyrsopteridaceae') & !is.na(spcode)) %>%
    ...
    mutate(
      ncensus = length(unique(year[status=='alive']))) %>%
    filter(ncensus >1) %>% 
    ...
    mutate(
      ...
      dbh_dt_relative = calculate_growth_rate(dbh, julian, log),
      ...
      dead_next_census = mortality_in_next_census(status),
      zombies = max(zombie_flagger(status), na.rm=T), # flags individuals that return from the dead. 
      datacheck = min(as.numeric(flag_bad_data(dbh, dbh_increment, dbh_dt)), na.rm=T)) %>% # CTFS datacheck (Dodgy data= 0). 
  ...
  # Rearranges columns
  individual_growth <- tbl_df(individual_growth) %>%
    select(latin,spcode,treeid,year,date,julian,census_interval,
           dbh,dbh_increment,dbh_dt,dbh_dt_relative,basal_area,basal_area_dt,
           basal_area_dt_relative,dead_next_census)
  
  individual_growth
  
  #FINAL COUNTS including NA's:
  #spp - 275
  #inds - 186393
  #obs - 715876 (with initial measurements); 530926 (without)
  #Total individual deaths - 52006 [(52006/186393)*100 = ~ 28%)]
}
...
BCI_calculate_species_traits <- function(individual_growth, wright_2010) {
  ...
  get_species_data <- function(individual_growth, min, max, at) {
    group_by(individual_growth, latin) %>%
      ...
      summarise(count = n(),
                dbh_dt = predict_via_quantile_regression(dbh, dbh_dt, predict_at = at),
...
figure_trait_growth_data <- function(X, Y, cex = 1, col = "darkgreen", pch = 1, ylim = range(Y, na.rm = TRUE), axes = c("lma", "dbh_dt"), ytick.lab = TRUE, ...) {
  if (ytick.lab){
...
figure_trait_growth_data_panel <- function(data, axes = c("lma", "dbh_dt"), ylim = c(1, 30)/1000, at = c(10, 20, 60, 120), title = FALSE, ...) {
...
fig_BCI_data <- function(data, at = c(10, 20, 50, 100, 200)/1000) {
  ...
  figure_trait_growth_data_panel(data, axes = c("lma", "dbh_dt"), at = at, title=TRUE, xlim= c(0.02, 0.2))
  header_plot(get_axis_info("lma", "lab"))
  
  figure_trait_growth_data_panel(data, axes = c("rho", "dbh_dt"), at = at)
  header_plot(get_axis_info("rho", "lab"))
  
  figure_trait_growth_data_panel(data, axes = c("hmat", "dbh_dt"), at = at)
  header_plot(get_axis_info("hmat", "lab"))
  
  mtext(get_axis_info("dbh_dt", "lab"), line = 4, side = 2, cex = 1, outer = TRUE)
  par(op)
}
