...
if (digest(d, algo="sha1") != hash.obj.sha)
  warning(paste("Imported data did not match expected hash; ",digest(d, algo="sha1")))
...
d <- cbind(d[-c(i.log)], d.unlogged)

# conevrt LL from months to years
d$LeafLifespan <- d$LeafLifespan/12
