...
figure_lma_tradeoff <- function(data){
  ...
  points(lma[i],leaf_turnover[i], type ='p', col=make.transparent("grey", 0.3),pch= 16, cex=0.9)

  sm1 <- sma(leaf_turnover[i]~lma[i]*sites[i], log='xy')
  plot(sm1, add= T, col=colour.by.category(data$biome[match(sm1$groups, sites)], col.table), type='l', lwd=1.0, p.lines.transparent=0.15)
...
figure_mass_fraction <- function(h=1:50, E=1, strategy = default_strategy(),
			xvar="height",
...
figure_trait_effect_at_size <- function(trait, growth_measure, size_measure, size,
	strategy=default_strategy(), E=1, ylim=NULL, title=TRUE, ylab=NULL, ...){
...
figure_trait_effect_at_sizes <- function(trait, growth_measure, size_measure, sizes, ...) {
	...
	for(size in sizes)
		figure_trait_effect_at_size(trait, growth_measure, size_measure, size, ...)
...
figure_trait_effects_at_sizes <- function(trait, size_measure, sizes,
	...
	for(growth_measure in growth_measures){
		...
		for(size in sizes){
			...
			figure_trait_effect_at_size(trait, growth_measure, size_measure, size, title=title,
...
figure_trait_relative_effects_at_sizes <- function(trait, size_measure, sizes,
	growth_measures=c("height_growth_rate_relative", "dbasal_area_dt_relative",
...
figure_trait_effects_at_heights <- function(trait,
	size_measure="height", sizes=c(0.25, 2, 8, 16),...){
	figure_trait_effects_at_sizes(trait, size_measure, sizes, ...)
}

figure_trait_relative_effects_at_heights <- function(trait,
	size_measure="height", sizes=c(0.25, 2, 8, 16),...){
	figure_trait_relative_effects_at_sizes(trait, size_measure, sizes, ...)
}

figure_trait_effects_at_diameters <- function(trait,
	size_measure="diameter", sizes=c(0.0025, 0.01, 0.1, 0.5),...){
	figure_trait_effects_at_sizes(trait, size_measure, sizes, ...)
}

figure_trait_relative_effects_at_diameters <- function(trait,
	size_measure="diameter", sizes=c(0.0025, 0.01, 0.1, 0.5),...){
	figure_trait_relative_effects_at_sizes(trait, size_measure, sizes, ...)
}
